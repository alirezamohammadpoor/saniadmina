...
(function() {
  "use strict";
  var _settings, _resizeObserver, _intersectionObserver, _onBlockSelect, _onBlockDeselect, _onClickBound, _onKeydownBound, _previouslyFocused, _ProductInfoModal_instances, focusTabByOffset_fn, _onInfoTriggerClick, _onFormSubmitBound, _onCardClick, _abortController, _isLoading, _state, _muteUpdateSync, _onToggleClick, _onDocumentKeydown, _onDocumentClick, _onLinkMouseEnter, _onLinkClick, _onPanelAnchorClick;
  ...
  const doComponentCleanup = (instance2) => {
    ...
    for (const value of Object.values(instance2)) {
  ...
  const _ScrollSection = class _ScrollSection extends BaseComponent {
    constructor(el) {
      super(el);
    ...
    handleClick(e) {
      ...
      const firstCard = this.container.querySelector(":scope > *");
  ...
  _ScrollSection.TYPE = "scroll-section";
  ...
  */
  ...
  const _VariantPickerOption = class _VariantPickerOption extends BaseComponent {
    constructor(el, options = {}) {
      ...
      const toggles = this.qsa("[data-toggle-system]");
      if (toggles.length) {
        toggles.forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            this.qsa("[data-size-eu]").forEach((el2) => el2.classList.toggle("hidden", system !== "eu"));
            this.qsa("[data-size-us]").forEach((el2) => el2.classList.toggle("hidden", system !== "us"));
            this.qsa("[data-size-uk]").forEach((el2) => el2.classList.toggle("hidden", system !== "uk"));
          });
        });
      }
    }
  ...
  const _ProductInfoModal = class _ProductInfoModal extends BaseComponent {
    constructor(el) {
      super(el);
      __privateAdd(this, _ProductInfoModal_instances);
      __privateAdd(this, _onClickBound);
      __privateAdd(this, _onKeydownBound);
      __privateAdd(this, _previouslyFocused, null);
      this.backdrop = this.qs("[data-modal-backdrop]");
      this.panel = this.qs("[data-modal-panel]");
      this.tabs = this.qsa("[data-info-tab]");
      this.panels = this.qsa("[data-info-panel]");
      __privateSet(this, _onClickBound, this.onClick.bind(this));
      __privateSet(this, _onKeydownBound, this.onKeydown.bind(this));
      this.el.addEventListener("click", __privateGet(this, _onClickBound));
      document.addEventListener("keydown", __privateGet(this, _onKeydownBound));
    }
    destroy() {
      this.el.removeEventListener("click", __privateGet(this, _onClickBound));
      document.removeEventListener("keydown", __privateGet(this, _onKeydownBound));
      super.destroy();
    }
    get isOpen() {
    ...
    open(tabId) {
      __privateSet(this, _previouslyFocused, document.activeElement);
    ...
    close() {
      ...
      setTimeout(() => {
        ...
        __privateGet(this, _previouslyFocused)?.focus?.();
    ...
    onKeydown(e) {
      ...
      if (focusedTab?.getAttribute("role") === "tab" && this.tabs.includes(focusedTab)) {
        switch (e.key) {
          ...
          case "ArrowRight":
            e.preventDefault();
            __privateMethod(this, _ProductInfoModal_instances, focusTabByOffset_fn).call(this, 1);
            break;
          case "ArrowUp":
          case "ArrowLeft":
            e.preventDefault();
  ...
  _onClickBound = new WeakMap();
  _onKeydownBound = new WeakMap();
  _previouslyFocused = new WeakMap();
  _ProductInfoModal_instances = new WeakSet();
  focusTabByOffset_fn = function(offset) {
  ...
  _ProductInfoModal.TYPE = "product-info-modal";
  let ProductInfoModal = _ProductInfoModal;
  const selectors$f = {
  ...
  const _ProductDetailForm = class _ProductDetailForm extends BaseComponent {
    ...
    constructor(el, options = {}) {
      super(el);
      __privateAdd(this, _onInfoTriggerClick);
      __privateAdd(this, _onFormSubmitBound);
      this.settings = {
      ...
      const infoModalEl = document.querySelector(ProductInfoModal.SELECTOR);
      this.infoModal = infoModalEl ? new ProductInfoModal(infoModalEl) : null;
      __privateSet(this, _onInfoTriggerClick, (e) => {
        const trigger = e.target.closest("[data-info-trigger]");
        if (trigger && this.infoModal) {
          this.infoModal.open(trigger.dataset.infoTrigger);
        }
      });
      this.el.addEventListener("click", __privateGet(this, _onInfoTriggerClick));
      this.a11yStatus = A11yStatus.generate(this.form);
      __privateSet(this, _onFormSubmitBound, this.onFormSubmit.bind(this));
      this.form.addEventListener("submit", __privateGet(this, _onFormSubmitBound));
    }
    destroy() {
      this.el.removeEventListener("click", __privateGet(this, _onInfoTriggerClick));
      this.form.removeEventListener("submit", __privateGet(this, _onFormSubmitBound));
      super.destroy();
    }
  ...
  _onInfoTriggerClick = new WeakMap();
  _onFormSubmitBound = new WeakMap();
  ...
  const _ProductDetailGallery = class _ProductDetailGallery extends BaseComponent {
    constructor(el) {
      ...
      this.pagination = this.qs(selectors$e.pagination);
      this.dots = this.qsa("[data-dot]");
      this.buttonNext = this.qs(selectors$e.buttonNext);
      this.buttonPrevious = this.qs(selectors$e.buttonPrevious);
  ...
  const _Backdrop = class _Backdrop extends BaseComponent {
    static generate(parent, options = {}) {
      ...
      el.classList.add(classes$5.backdrop);
    ...
    show() {
      this.el.classList.add(classes$5.open);
      this.el.setAttribute("aria-hidden", toAriaBoolean(false));
    }
    hide() {
      this.el.classList.remove(classes$5.open);
  ...
  const _Drawer = class _Drawer extends BaseComponent {
    constructor(el, options = {}) {
      ...
      this.scroller = this.qs(selectors$d.scroller);
      this.backdrop = null;
    ...
    open() {
      if (this.isOpen) return;
      this.el.classList.add(classes$4.isOpen);
      this.el.setAttribute("aria-hidden", toAriaBoolean(false));
      this.el.setAttribute("aria-modal", toAriaBoolean(true));
      document.body.classList.add(classes$4.bodyIsOpen);
    ...
    close() {
      if (!this.isOpen) return;
      this.el.classList.remove(classes$4.isOpen);
      this.el.setAttribute("aria-hidden", toAriaBoolean(true));
      this.el.setAttribute("aria-modal", toAriaBoolean(false));
      this.backdrop?.hide();
      document.body.classList.remove(classes$4.bodyIsOpen);
    ...
    onClick(e) {
      const target = e.target;
      if (target?.closest(selectors$d.close)) {
        e.preventDefault();
        this.close();
      }
    }
  ...
  const _ColorPickerDrawer = class _ColorPickerDrawer extends Drawer {
    constructor(el) {
      super(el, { backdrop: true });
      __privateAdd(this, _onCardClick);
      __privateSet(this, _onCardClick, this.onCardClick.bind(this));
      this.el.addEventListener("click", __privateGet(this, _onCardClick));
    }
    destroy() {
      this.el.removeEventListener("click", __privateGet(this, _onCardClick));
      super.destroy();
    }
    onCardClick(e) {
      const target = e.target;
      const card = target.closest(selectors$c.card);
      if (!card || card.disabled) return;
      const color = card.dataset.color;
      if (!color) return;
      const radios = document.querySelectorAll(selectors$c.colorRadio);
  ...
  _onCardClick = new WeakMap();
  _ColorPickerDrawer.TYPE = "color-picker-drawer";
  let ColorPickerDrawer = _ColorPickerDrawer;
  const _ProductSection = class _ProductSection extends BaseSection {
    constructor(container) {
      ...
      const colorPickerEl = this.qs(ColorPickerDrawer.SELECTOR);
  ...
  class BaseRenderer extends Renderer {
    ...
    onEnter() {
      ...
      document.querySelectorAll(ScrollSection.SELECTOR).forEach((el) => new ScrollSection(el));
    }
    onEnterCompleted() {
  ...
  const _HeaderCartControl = class _HeaderCartControl extends BaseComponent {
    ...
    onCartUpdate(e) {
      ...
      this.el.classList.toggle(classes$3.hasItems, cart.item_count > 0);
    }
  };
  ...
  const _NewsletterForm = class _NewsletterForm extends BaseComponent {
    ...
    showMessageWithTimeout(message, reset = false) {
      ...
      this.formContents.classList.add(classes$2.showMessage);
      window.clearTimeout(this.timeoutId);
      this.timeoutId = setTimeout((function() {
        if (reset) {
          this.reset();
        }
        this.formContents.classList.remove(classes$2.showMessage);
      }).bind(this), 3e3);
    }
    showFormContents() {
      this.form.classList.add(classes$2.showContents);
      setTimeout(() => {
        this.focusInput();
      }, 100);
    }
    hideFormContents() {
      this.form.classList.remove(classes$2.showContents);
    }
  ...
  const _MobileMenuDrawer = class _MobileMenuDrawer extends Drawer {
    constructor(el) {
      ...
      const searchForm = this.qs("[data-search-form]");
  ...
  const _QuantityAdjuster = class _QuantityAdjuster extends BaseComponent {
    constructor(el, options = {}) {
      ...
      this.increment = this.el.querySelector(selectors$6.increment);
      this.decrement = this.el.querySelector(selectors$6.decrement);
  ...
  const _CartItem = class _CartItem extends BaseComponent {
    constructor(el, itemData, options = {}) {
      ...
      this.remove = this.qs(selectors$5.remove);
      this.price = this.qs(selectors$5.price);
  ...
  const _CartBody = class _CartBody extends BaseComponent {
    constructor(el, cartData) {
      ...
      this.list = this.qs(selectors$4.list);
  ...
  const _CartFooter = class _CartFooter extends BaseComponent {
    constructor(el) {
      ...
      this.submit = this.qs(selectors$3.submit);
      this.subtotalPrice = this.qs(selectors$3.subtotalPrice);
    }
    onCartUpdate(e) {
      const { cart } = e.detail;
  ...
  const _AJAXCart = class _AJAXCart extends BaseComponent {
    ...
    onBodyClick(e) {
      const target = e.target;
      if (target?.closest(selectors$2.close)) {
  ...
  const _AJAXCartSection = class _AJAXCartSection extends BaseSection {
    constructor(container) {
      super(container);
      const cartJsonEl = this.qs(selectors$1.cartJson);
  ...
  const _AnnouncementRotator = class _AnnouncementRotator extends BaseComponent {
    constructor(el) {
      super(el);
      ...
      if (this.messages.length > 1) {
        const speed = parseInt(this.el.dataset.speed || "4", 10) * 1e3;
  ...
  _AnnouncementRotator.TYPE = "announcement-rotator";
  let AnnouncementRotator = _AnnouncementRotator;
  const STORAGE_KEY = "sania-currency";
  const _CurrencySelector = class _CurrencySelector extends BaseComponent {
    constructor(el) {
      super(el);
      this.toggle = el.querySelector("[data-currency-toggle]");
      this.dropdown = el.querySelector("[data-currency-dropdown]");
      this.label = el.querySelector("[data-currency-label]");
      this.options = Array.from(el.querySelectorAll("[data-currency-option]"));
      ...
      this.options.forEach((option) => {
        option.addEventListener("click", () => {
          const value = option.dataset.currencyOption;
          this.select(value);
        });
      });
  ...
  _CurrencySelector.TYPE = "currency-selector";
  let CurrencySelector = _CurrencySelector;
  const selectors = {
    link: "[data-menu-link]",
  ...
  const _DesktopMenu = class _DesktopMenu extends BaseComponent {
    constructor(el) {
      super(el);
      __privateAdd(this, _onToggleClick);
      __privateAdd(this, _onDocumentKeydown);
      __privateAdd(this, _onDocumentClick);
      __privateAdd(this, _onLinkMouseEnter);
      __privateAdd(this, _onLinkClick);
      __privateAdd(this, _onPanelAnchorClick);
      this.panel = el;
      this.toggle = this.panel.id ? document.querySelector(`[aria-controls="${this.panel.id}"]`) : null;
      this.links = Array.from(this.panel.querySelectorAll(selectors.link));
      ...
      const defaultImgEl = this.previewImage?.querySelector(selectors.defaultImage);
      ...
      this.isOpen = false;
      __privateSet(this, _onToggleClick, this.onToggleClick.bind(this));
      __privateSet(this, _onDocumentKeydown, this.onDocumentKeydown.bind(this));
      __privateSet(this, _onDocumentClick, this.onDocumentClick.bind(this));
      __privateSet(this, _onLinkMouseEnter, this.onLinkMouseEnter.bind(this));
      __privateSet(this, _onLinkClick, this.onLinkClick.bind(this));
      __privateSet(this, _onPanelAnchorClick, this.onPanelAnchorClick.bind(this));
      this.toggle?.addEventListener("click", __privateGet(this, _onToggleClick));
      ...
      for (const link of this.links) {
        link.addEventListener("mouseenter", __privateGet(this, _onLinkMouseEnter));
        link.addEventListener("click", __privateGet(this, _onLinkClick));
      }
      for (const a of Array.from(this.panel.querySelectorAll("a"))) {
        a.addEventListener("click", __privateGet(this, _onPanelAnchorClick));
      }
    }
    destroy() {
      this.toggle?.removeEventListener("click", __privateGet(this, _onToggleClick));
      ...
      for (const link of this.links) {
        link.removeEventListener("mouseenter", __privateGet(this, _onLinkMouseEnter));
        link.removeEventListener("click", __privateGet(this, _onLinkClick));
      }
      for (const a of Array.from(this.panel.querySelectorAll("a"))) {
        a.removeEventListener("click", __privateGet(this, _onPanelAnchorClick));
      }
      super.destroy();
    }
    open() {
    ...
    onLinkMouseEnter(e) {
      ...
      if (!handle) return;
      const cached = this.hiddenImages.querySelector(`[data-collection="${handle}"]`);
      if (!cached) return;
      const img = this.previewImage.querySelector("img");
      if (img) {
        img.style.opacity = "0";
  ...
  _onLinkMouseEnter = new WeakMap();
  _onLinkClick = new WeakMap();
  _onPanelAnchorClick = new WeakMap();
  _DesktopMenu.TYPE = "desktop-menu";
  let DesktopMenu = _DesktopMenu;
  ...
  function init() {
    ...
    for (const el of document.querySelectorAll(AnnouncementRotator.SELECTOR)) {
      new AnnouncementRotator(el);
    }
    for (const el of document.querySelectorAll(CurrencySelector.SELECTOR)) {
      new CurrencySelector(el);
    }
    for (const el of document.querySelectorAll(ScrollSection.SELECTOR)) {
      new ScrollSection(el);
    }
    for (const el of document.querySelectorAll(DesktopMenu.SELECTOR)) {
      new DesktopMenu(el);
    }
    if (isThemeEditor()) {
    ...
    taxi.on("NAVIGATE_IN", (e) => {
      ...
      for (const cn of Array.from(toPage.body.classList)) {
        if (TEMPLATE_REGEX.test(cn)) {
